...
function EmailConfirmModal({ isOpen, onClose, onConfirm, loading, employeeName, email, isEdit }) {
        ...
        <div className="px-6 py-6 flex flex-col items-center text-center gap-4">
          ...
          <div>
            <p className="text-base font-bold text-gray-900 mb-1">
              {isEdit ? 'Submit Employee?' : 'Send Credentials?'}
            </p>
            <p className="text-sm text-gray-500 leading-relaxed">
              This will {isEdit ? 'finalise the record' : 'create the employee record'} for{' '}
...
export default function AddEmployee() {
  ...
  // ── FIX: Mode detection — use startsWith/endsWith to avoid false positives ──
  // ROUTES.EMPLOYEE_DRAFT = '/employee/:id/draft'  → resolved: /employee/123/draft
  // ROUTES.EMPLOYEE_EDIT  = '/employee/:id/edit'   → resolved: /employee/123/edit
  // The old code used includes('/draft/') which never matched because the URL
  // ends with '/draft' (no trailing slash).
  const isDraftMode = !!id && location.pathname.endsWith('/draft')
  const isEditMode  = !!id && location.pathname.endsWith('/edit')
  const isAddMode   = !id

  // FIX: page title now correctly reflects mode
  const pageTitle = isEditMode  ? 'Edit Employee'
  ...
  // ── Load existing employee data (edit/draft modes) ────────────────────────
  // FIX: This effect now correctly maps ALL fields from the API response,
  //      including the nested contact sub-object and reportingManagerName.
  //      Dropdown IDs are resolved by matching names against loaded options.
  useEffect(() => {
    if (!id) return
    const load = async () => {
      setPageLoading(true)
      try {
        ...
        // FIX: contact fields live in emp.contact sub-object
        ...
        // NOTE: designationId/departmentId/branchId/shiftId are NOT in the DTO —
        //       we store the names and resolve IDs later when dropdowns are loaded.
        setOffice({
          employeeId:        emp.employeeCode                          ?? '',
          designation:       emp.designationName                       ?? '',
          designationId:     null,   // resolved after dropdowns load
          department:        emp.departmentName                        ?? '',
          departmentId:      null,   // resolved after dropdowns load
          // FIX: officeEmail is in the contact object
          officeEmail:       contact.officeEmail                       ?? '',
          joiningDate:       emp.employment?.dateOfJoining             ?? '',
          experience:        emp.employment?.yearOfExperience != null
                               ? String(emp.employment.yearOfExperience) : '',
          ...
          workLocationId:    null,   // resolved after dropdowns load
          shiftId:           null,   // resolved after dropdowns load
          shiftName:         emp.shiftTiming                           ?? '',
          // FIX: API returns reportingManagerName (first name only from BaseUserMapper)
          reportingManager:  emp.reportingManagerName                  ?? '',
          role:              emp.role                                  ?? '',
        })

        setEmpDetails({
          salary:      emp.employment?.ctc != null
                         ? String(emp.employment.ctc) : '',
          noticePeriod:emp.employment?.noticePeriod != null
                         ? String(emp.employment.noticePeriod) : '',
        ...
        // FIX: Load existing documents for this employee in edit/draft mode
  ...
  // FIX: After dropdowns load, resolve IDs by matching names (since API only returns names)
  // This runs whenever both the dropdown lists and the office state update.
  useEffect(() => {
  ...
  const checkEmailsAvailable = async () => {
    ...
    try {
      if (office.employeeId.trim()) {
        ...
        if (!available) issues.push({ field: 'employeeId', msg: 'Employee ID is already in use' })
      }
    } catch { /* network fail — backend will catch */ }
    return issues
  }
  ...
  const buildPayload = useCallback((isDraft) => {
    ...
    if (Object.keys(reasonsMap).length > 0) fd.append('reasons', JSON.stringify(reasonsMap))
      ...
      <div className="min-h-full pb-8">
        ...
        <div className="flex items-start justify-between mb-5 flex-wrap gap-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {!isAddMode && <div className="h-5 w-px bg-gray-200" />}

            {/* FIX: pageTitle now correctly shows "Edit Draft" or "Edit Employee" */}
            <h1 className="text-xl font-bold text-gray-900">{pageTitle}</h1>

            {/* FIX: draft badge correctly shown only in isDraftMode */}
            {isDraftMode && (
        ...
        <div className="flex flex-col gap-5">
          ...
          <SectionCard title="Personal Information">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
              ...
              <div data-error={!!errors.personalEmail}>
                ...
                {/* FIX: value mapped from contact.personalEmail in edit load */}
          ...
          <SectionCard title="Office Information">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
              <div>
                <FieldLabel>Employee ID {isAddMode && '(auto-generated if blank)'}</FieldLabel>
                <TextInput
                  placeholder="GMEP001"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
              <div data-error={!!errors.officeEmail}>
                ...
                {/* FIX: value mapped from contact.officeEmail in edit load */}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
              ...
              <div data-error={!!errors.reportingManager}>
                ...
                {/* FIX: value mapped from emp.reportingManagerName in edit load */}
          ...
          <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 pt-2">
            ...
            >
              ...
              {isAddMode ? 'Add Employee' : 'Submit Employee'}
            </button>
          </div>
