// src/pages/employee/ViewIntern.jsx
// Route: /intern/:id/view  (id = personalInformationId)
...
import {
  ...
  Clock, AlertCircle, ExternalLink, BookOpen, Users, Star,
  GraduationCap,
} from 'lucide-react'
  ...
  return (
    <div className="mt-3">
      <div className="flex items-center justify-between mb-1.5">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Internship Progress</p>
      ...
      <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-all duration-500"
          style={{ width: `${pct}%`, backgroundColor: done ? '#16A34A' : PRIMARY }} />
      </div>
  ...
  return (
    <div className={`rounded-xl border-2 p-3.5 flex flex-col gap-2 transition-colors ${
      ...
      {hasFile ? (
        ...
        <p className="text-[10px] text-amber-600 pl-10">File unavailable</p>
      ) : reason ? (
...
export default function ViewIntern() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // id = personalInformationId — backend dispatches by type
        ...
        toast.error(err?.response?.data?.message || 'Failed to load intern')
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-gray-500">Loading intern profile…</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-full pb-10">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div className="flex items-center gap-3 flex-wrap">
          <button onClick={() => navigate(ROUTES.EMPLOYEE)}
        ...
        {canEdit && (
          <button
            onClick={() => navigate(
              isDraft
                ? ROUTES.EMPLOYEE_INTERN_DRAFT.replace(':id', id)
                : ROUTES.EMPLOYEE_INTERN_EDIT.replace(':id', id)
            )}
          ...
          >
            <Pencil size={14} />
            {isDraft ? 'Continue Editing' : 'Edit Intern'}
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl border-2 overflow-hidden" style={{ borderColor: BORDER }}>
          <div className="relative px-5 pt-5 pb-5 flex flex-col sm:flex-row sm:items-center gap-4"
            style={{
            ...
            <div className="relative flex-shrink-0 z-10">
              {profileImageUrl ? (
                ...
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl border-4 shadow-lg flex items-center justify-center text-xl sm:text-2xl font-bold text-white flex-shrink-0"
          ...
          <div className="px-5 py-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { icon: <Calendar  size={14} />, label: 'Start Date',  value: fmtDate(internshipDetails?.startDate) },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            <SectionCard title="Personal Information" icon={<User size={15} />}>
              ...
              <Divider />
              <p className="text-[10px] font-bold uppercase tracking-widest mb-3" style={{ color: PRIMARY }}>Contact</p>
              <FieldGrid cols={2}>
            ...
            <SectionCard title="Office Information" icon={<Briefcase size={15} />}>
              <FieldGrid cols={3}>
                <Field label="Intern ID"         value={fmt(internCode)}         highlight />
            ...
            {/* Internship Details */}
            {internshipDetails && (
              <SectionCard title="Internship Details" icon={<Clock size={15} />}>
                <FieldGrid cols={3}>
                  <Field label="Domain"       value={fmt(internshipDetails.domainName)}         />
                  ...
                  <Field label="Duration"
                    value={internshipDetails.trainingPeriodMonths
                      ? `${internshipDetails.trainingPeriodMonths} month(s)` : '—'} />
                  <Field label="Type"
                    value={internshipDetails.internshipType === 'PAID' ? 'Paid' : 'Unpaid'} />
                  <Field label="Stipend"
                    value={internshipDetails.internshipType === 'PAID'
                      ? fmtCurrency(internshipDetails.stipend) : 'Unpaid'} />
                </FieldGrid>
                <InternshipProgress
            ...
            {/* College Details */}
            {collegeDetails && (
              <SectionCard title="College Information" icon={<GraduationCap size={15} />}>
                <FieldGrid cols={3}>
                  <Field label="Course Name"      value={fmt(collegeDetails.courseName)}       />
                  <Field label="Semester"         value={fmt(collegeDetails.semester)}          />
                  <Field label="Academic Year"    value={fmt(collegeDetails.academicYear)}      />
                  <Field label="Enrollment No."   value={fmt(collegeDetails.enrollmentNumber)}  />
                  <Field label="College Name"     value={fmt(collegeDetails.collegeName)}  span={2} />
                  <Field label="College Address"  value={fmt(collegeDetails.collegeAddress)} span={3} />
                  <Field label="University"       value={fmt(collegeDetails.universityName)} span={2} />
                  <Field label="Degree Completion"
                    value={[collegeDetails.degreeCompletionStatus, collegeDetails.year]
                      .filter(Boolean).join(' ') || '—'} />
                </FieldGrid>
              </SectionCard>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="flex items-start gap-3 p-3.5 rounded-xl border"
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-900">{mentorDetails.mentorName ?? '—'}</p>
                  ...
                  {(mentorDetails.supervisorName || mentorDetails.supervisorEmployeeId) && (
                    <div className="flex items-start gap-3 p-3.5 rounded-xl border"
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-gray-900">{mentorDetails.supervisorName ?? '—'}</p>
          ...
          <div className="flex flex-col gap-5">
            ...
            {/* Internship Summary */}
            <SectionCard title="Internship Summary" icon={<BookOpen size={15} />}>
              <div className="space-y-0">
                <InfoRow label="Status"  value={statusInfo.label} />
                <InfoRow label="Domain"  value={internshipDetails?.domainName} />
                <InfoRow label="Type"
                  value={internshipDetails?.internshipType === 'PAID' ? 'Paid' : 'Unpaid'} />
                {internshipDetails && (
                  <>
                    <InfoRow label="Duration"
                      value={internshipDetails.trainingPeriodMonths
                        ? `${internshipDetails.trainingPeriodMonths} month(s)` : '—'} />
                    {internshipDetails.internshipType === 'PAID' && (
                      <InfoRow label="Stipend" value={fmtCurrency(internshipDetails.stipend)} />
                    )}
                    <InfoRow label="Start Date" value={fmtDate(internshipDetails.startDate)} />
                    <InfoRow label="End Date"   value={fmtDate(internshipDetails.endDate)} last />
                  </>
                )}
              </div>
            </SectionCard>
            ...
            {/* College snapshot */}
            {collegeDetails && (
              <SectionCard title="College Snapshot" icon={<GraduationCap size={15} />}>
                <div className="space-y-0">
                  <InfoRow label="Course"     value={collegeDetails.courseName}    />
                  <InfoRow label="Semester"   value={collegeDetails.semester}       />
                  <InfoRow label="University" value={collegeDetails.universityName} />
                  <InfoRow label="Enrollment" value={collegeDetails.enrollmentNumber} />
                  <InfoRow label="Completion"
                    value={[collegeDetails.degreeCompletionStatus, collegeDetails.year]
                      .filter(Boolean).join(' ') || '—'} last />
                </div>
              </SectionCard>
            )}
          </div>
        ...
        <SectionCard title="Documents" icon={<FileText size={15} />}>
          {docLoading ? (
            ...
            <p className="text-sm text-gray-400 py-2">No documents found for this intern.</p>
          ) : (
          ...
          <div className="flex items-start gap-3 px-5 py-4 rounded-2xl border-2 border-amber-200 bg-amber-50">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-600 mt-0.5">
                This intern profile is incomplete. Complete all required fields and submit to activate the account.
              </p>
            </div>
            {canEdit && (
              <button
                onClick={() => navigate(ROUTES.EMPLOYEE_INTERN_DRAFT.replace(':id', id))}
