...
import { Users, Clock, UserMinus, FolderOpen, MessageSquare } from 'lucide-react'
...
const STATS = [
  ...
  { label: 'Total Projects',  value: 13, Icon: FolderOpen },
]
...
const PROJECT_DATA = [
  { name: 'Ready For Handover', value: 100, color: '#6366F1' },
  { name: 'On Hold',            value: 20,  color: '#EF4444' },
  { name: 'Completed',          value: 62,  color: '#22C55E' },
  { name: 'In Progress',        value: 50,  color: '#F97316' },
  { name: 'Not Started',        value: 28,  color: '#06B6D4' },
]
...
const renderProjectLabel = ({ cx, cy, midAngle, outerRadius, name, value, fill }) => {
  const r = outerRadius + 38
  const x = cx + r * Math.cos(-(midAngle * Math.PI) / 180)
  const y = cy + r * Math.sin(-(midAngle * Math.PI) / 180)
  return (
    <text x={x} y={y} textAnchor={x > cx ? 'start' : 'end'} fill={fill} fontSize={9.5}>
      <tspan x={x} dy="-0.55em">{name}</tspan>
      <tspan x={x} dy="1.5em" fontWeight="700" fontSize={13}>{value}</tspan>
    </text>
  )
}

// ─── Chart Card (uses shared DateFilter) ─────────────────────────────────────
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-full box-border">
      <h1 className="m-0 mb-5 text-[22px] font-bold text-text font-display">Dashboard</h1>
      ...
      <div className={isLg ? 'flex gap-5 items-start' : 'block'}>
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          {/* Row 2: Employee Overview + Attendance */}
          ...
          <ChartCard title="Project Status Graph">
            <ResponsiveContainer width="100%" height={340}>
              <PieChart>
                <Pie data={PROJECT_DATA} cx="50%" cy="47%"
                  innerRadius={65} outerRadius={105} paddingAngle={2} dataKey="value"
                  label={renderProjectLabel} labelLine={{ stroke: '#D1D5DB', strokeWidth: 1 }}>
                  {PROJECT_DATA.map((item, i) => <Cell key={i} fill={item.color} />)}
                  <Label content={({ viewBox }) => {
                    const { cx, cy } = viewBox
                    return <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central"
                      fontSize={28} fontWeight={700} fill="#111827">240</text>
                  }} />
                </Pie>
                <Tooltip contentStyle={ttStyle} />
                <Legend iconType="circle" iconSize={8} layout="horizontal"
                  formatter={(v) => <span style={{ color: '#6B7280', fontSize: 10 }}>{v}</span>}
                  payload={PROJECT_DATA.map(d => ({ value: d.name, type: 'circle', color: d.color }))}
                />
              </PieChart>
            </ResponsiveContainer>
          </ChartCard>
